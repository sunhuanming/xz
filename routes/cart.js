...
router.get("/add",(req,res)=>{
  ...
  pool.query("select * from xz_shoppingcar_item where user_id=? and product_id=?",[uid,lid],(err,result)=>{
    if(err) console.log(err);
    if(result.length==0){
        pool.query("insert into shoppingcar_item value(null,?,?,?)",[uid,lid,count],(err,result)=>{
          if(err) console.log(err);
          res.end();
          })
      })
    }else{
      pool.query("updata xz_shoppingcar_item set count=count+? where user_id=? and product_id=?",[count,uid,lid],(err,result)=>{
        if(err) console.log(err);
        res.end();
      })
    }
  ...
  router.get("/updata",(req,res)=>{
    ...
    var count=req.query.count;
    if(count>0){
      var sql=`update xz_shoppingcar_item set count=? where iid=?`;
      var data=[count,iid];
    }else{
      var sql=`delete from xz_shoppingcar_item where iid=?`;
      var sql=[iid];
    }
